...
switch fcategory
    case {'custom','library'}
        ...
        if ~islinear(model) % nonlinear custom or library equation
            ...
            try
                [xout,resnorm,res,exitflag,optoutput,lam,convmsg,J] = iNonLinearFit(...
            ...
            if dorobust
                try
                    [p,s,~,lam,optoutput,convmsg, exitflag] = iRobustLinearFit( ...
...
function [xout,resnorm,res,exitflag,optoutput,lam,convmsg,J] = iNonLinearFit(...
    model, start,xdata,wtdydata,lowerbnd,upperbnd, options,probparams, ...
    separargs,weights, optimargs, nonlcoeffindex)
