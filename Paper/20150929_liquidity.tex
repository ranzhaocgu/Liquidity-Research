...
%TCIDATA{LastRevised=Friday, October 02, 2015 09:08:56}
...
matrix. Namely, what are $b_{h}$ and $b_{\eta }$? The key observation is
that, while there are $S$ different values of $q(p)$, there are $S+1$
different (independent)\ values for $Q(p)$, so that there should be $S+1$
Brownian motions, which I\ label $W_{0},..,W_{S\text{.}}$. Define
...
so that:%
\begin{eqnarray*}
\frac{dh_{i}}{h_{i}} &=&[..]dt+\sigma _{h_{i}}\sum_{j=0}^{S}b_{ij}^{h}dW_{j}
\\
d\eta  &=&[..]dt+\sqrt{\eta (1-\eta )}\sigma _{\eta }\sum_{s=0}^{S}b_{\eta
,j}(s)W(ds,dt,\omega )
\end{eqnarray*}

A possible choice is to set $b_{\eta ,0}=1$, and all the others zero, so
that:%
\begin{eqnarray*}
b_{ij}^{h}dt &=&Corr[\frac{dh_{i}}{h_{i}},\frac{dh_{j}}{h_{j}}]\text{ \ \ \ }%
1\leq i,j\leq S \\
b_{0j}^{h}dt &=&Corr[d\eta ,\frac{dh_{j}}{h_{j}}]\text{ \ \ \ }1\leq j\leq S
\end{eqnarray*}
