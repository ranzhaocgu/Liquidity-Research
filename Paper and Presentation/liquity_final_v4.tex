...
\textbf{Remark}. Observe that, in this model, atomistic traders are myopic
...
\textbf{Lemma }There exists a mapping $P:$ $[\tau ,T]\times \lbrack
...
\textbf{Proof: }We fix $t,\omega $. With obvious notation, (\ref%
...
\textbf{Definitions }We call $\ \Xi (t,\omega )$ the set of all $x$ such
...
\textbf{Definitions \ }The \textit{asymptotic liquidation proceeeds} of the
...
\textbf{Theorem} Suppose assumptions 1 to 9 hold. If the market price of
...
exists a local martingale measure $\mathbb{Q}$ for $P(x,t)$. As in lemma 2,
...
We set $dQ(P(x,t),t)=0$ as in lemma 2. We insert (\ref{MPR_star})\ in (\ref%
...
\textbf{Remark} x To make it twice differentiable in price, we build $Q$
...
double-barrier process in (\ref{equ_eta_A}). Examination of (\ref{equ_Q_A})
...
\textbf{Remark }The reason why the support for the factor loadings in (\ref%
...
\textbf{Proposition:} Our model satisfies assumptions 4, 7, 8, and 9 are
satisfied. Also, the market price of risk equations (\ref{MPR}) hold.

\bigskip

\section{Empirical Analysis}

We assume in this section that all traders are atomistic. As stated in the
introductionIn empirical applications, it is difficult from looking at order
books to know which trader is a large trader, however it is possible to
assess with a reasonable degree of confidence whether orders are atomistic
or not. \bigskip
...
where $W(\Delta s, \Delta t) = \phi \sqrt{\Delta s \Delta t}$ and $\phi$ is
a standard normal random number. $\Delta s$ and $\Delta t$ are the step
length in price and in time respectively.
...
p,2\Delta p,..,S\Delta p\}$ with $S=401$ (winsorized at 99\% quantile of the
price for all the limit order within the trading day), and $\Delta p=\$1.91$%
. Time takes values in the set $\mathcal{T}=\{-N\Delta t,-(N-1)\Delta
t,..,0\}$, and the number of observations is equal to $N=32$. Therefore each
time step represents a time decay of 15 minutes. We thus use all the dates
between April 1st 9 AM and April 1st 5PM, which covers the regular trading
hours during the day. The net demand $Q$ at price $p$ is calculated, by
definition as:
