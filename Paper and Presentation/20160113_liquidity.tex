...
\begin{eqnarray*}
h_{0}(t+\Delta t)-h_{0}(t) &=&(\mu _{h_{0}}+a_{h_{0}}(h_{0}(t)-\bar{h}%
_{0}))\Delta t+\sigma _{h_{0}}(b_{h_{0,0}}(z(1,t)\sqrt{\Delta t}\sqrt{\Delta
s}-\lambda (0,0)\Delta t\Delta s)+...+b_{h_{0,S}}(z(S+1,t)\sqrt{\Delta t}%
\sqrt{\Delta s}-\lambda (S,0)\Delta t\Delta s)) \\
&&.. \\
h_{S-2}(t+\Delta t)-h_{S-2}(t) &=&(\mu _{h_{S-2}}+a_{h_{S-2}}(h_{S-2}(t)-%
\bar{h}_{S-2}))dt+\sigma _{h_{S-2}}(b_{h_{S-2,0}h_{0,0}}(z(1,t)\sqrt{\Delta t%
}\sqrt{\Delta s}-\lambda (0,0)\Delta t\Delta s)+...+b_{h_{S-2,S}}(z(S+1,t)%
\sqrt{\Delta t}\sqrt{\Delta s}-\lambda (S,0)\Delta t\Delta s)) \\
&&\text{and so forth} \\
&& \\
\frac{dq(0,t)}{q(0,t)} &=&(\mu _{q_{0}}+a_{q_{0}}(q(0,t)-\bar{q}%
(0,t))dt+\sigma _{q_{0}}(b_{q_{0,0}}\delta
...
\bigskip In discrete time this means
...
\textbf{Determination of missing parameters (Dec 14 2015)}

Once the parameters $\sigma _{h_{0}},...,\sigma _{h_{S-2}}$ as well as $%
b_{h_{S-2,0}}$,...,$b_{h_{S-2,S}}$ are known, we can calculate.$\sigma
_{q_{0}}$ and $b_{q_{0,0}},...,b_{q_{0,S}}:$

\begin{equation*}
\hat{\sigma}_{q_{0}}=\text{ historical volatility of }q(0,.)
\end{equation*}

For each $t_{i}$ we solve the following linear system for $\delta
_{s}W_{0}(t_{i})$,...,$\delta _{s}W_{S-2}(t_{i})$

\begin{equation*}
\left[ 
\begin{array}{cccc}
\sigma _{h_{0}}b_{h_{0,0}} &  &  & \sigma _{h_{0}}b_{h_{0,S-2}} \\ 
&  &  &  \\ 
&  &  &  \\ 
\sigma _{h_{S-2}}b_{h_{S-2,0}} &  &  & \sigma _{h_{S-2}}b_{h_{S-2,S-2}}%
\end{array}%
\right] \left[ 
\begin{array}{c}
d\delta _{s}W_{0}(t_{i}) \\ 
\\ 
\\ 
d\delta _{s}W_{S-2}(t_{i})%
\end{array}%
\right] =\left[ 
\begin{array}{c}
h_{0}(t_{i+1})-h_{0}(t_{i}) \\ 
\\ 
\\ 
h_{S-2}(t_{i+1})-h_{S-2}(t_{i})%
\end{array}%
\right]
\end{equation*}

Then given the time series for $q(0,t_{i})/\hat{\sigma}_{q_{0}}$, the
coefficients $b_{q_{0,0}},...,b_{q_{0,S-2}}$ are the regression coefficients
of $q(0,.)/\hat{\sigma}_{q_{0}}$ on d$\delta _{s}W_{0}(t_{i}),....,d\delta
_{s}W_{S-2}(t_{i}).$ We can determine $b_{q_{0,S-1}}$ by the equation

\begin{equation*}
\sum_{s=0}^{S-1}b_{q_{0},s}^{2}=1
\end{equation*}

Then the value of $\delta _{s}W_{S-1}(t_{i})$ is determined.by the equation:

\begin{equation*}
\frac{dq(0,t)}{q(0,t)}-\sigma _{q_{0}}d(b_{q_{0,0}}\delta
_{s}W_{0}(t)+...+b_{q_{0,S-2}}\delta _{s}W_{S-2}(t))=\hat{\sigma}%
_{q_{0}}b_{q_{0,S-1}}\delta _{s}W_{S-1}(t))
\end{equation*}

Set $b_{q_{0,S}}=0$. Then do the same thing for $Q$.
