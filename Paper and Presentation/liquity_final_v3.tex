...
%TCIDATA{LastRevised=Friday, May 20, 2016 19:01:30}
...
financial markets community \cite{WW02}. Our theoretical framework accommodates
...
high-frequency trading activity \cite{Eng00,BLT06,Hau08,BRZ} [SM08]
...
The first class of models (\cite{Jar92, Jar94, PS98a, PS98b, Fre98,
...
price-takers. In particular, \cite{CJP04} introduced an exogenous residual supply
...
instantaneity, it is plausible for \cite{CJP04} to assume the "price effect of an
order is limited to the very moment when the order is placed in the market"
(dixit \cite{BB04}), so that that the residual supply curve at a future time is
...
we extend the \cite{BB04} paper, where the clearing price is not characterized,
in the direction followed by \cite{CJP04}, where the clearing price is
characterized.
...
(t) $, then, under some mild assumptions there is no arbitrage, as in \cite{BB04}.

We do not claim that it is indispensable to use a Brownian sheet to model an
arbitrage-free limit order market. In an ingenious construction \cite{KR09} use
Carmona-Nualart [CT90] stochastic integration to weaken the assumptions in
the \cite{BB04} necessary to avoid arbitrage. They show for instance that a
...
\cite{BS73}\ result in a smile curve for implied volatility. Although limited,
...
consistent with the theory of optimal order book placement suggested by
\cite{Ros09}.
...
Da Prato and Zabczyk \cite{DPZ92}, or Krylov's \cite{Kry09} $L_{p}$ theory". \cite{DPZ92}
and [Mue09] note that the approach is at least as general as the [Wal86]
approach. We use the Hilbert space approach as exposed in \cite{CT06}.
...
as developed in \cite{Kry09}. Finally, we note that in Walsh's approach the
...
\textbf{Assumption 1 }(Assumption A1 in\textbf{\ }\cite{Jar92}).\ The market is
frictionless.
...
\textbf{Remark:} obviously when the order quantity is unity for every order,
the quantity of shares submitted for purchase is equal to the number of buy
orders. We use the term "quantity of shares submitted for purchase" because
it is smaller than "the sum of all the order buy quantities across all buy
orders".
...
\textbf{Definition:} As in \cite{KR09} we denote by $S_{A}(\theta ,t)$ the price
...
\textbf{Definition }\cite{BB04}\textbf{\ }The asymptotic liquidation proceeeds of
...
\textbf{Fact:}\ (lemma 3.2 in \cite{BB04}) Provided that the $L(\theta )$ is a
smooth family of semimartingales (in the sense of \cite{BB04}), for any
...
sense of definition 2.2 in \cite{BB04}), and satisfies:
...
\begin{eqnarray}
dh(p,t,\omega ) &=&(-a_{h}(p)h(p,t,\omega )+\mu _{h}(h(p,t,\omega
...
d\eta (t,\omega ) &=&a_{\eta }(\bar{\eta}-\eta (t,\omega ))dt+\sqrt{\eta
(t,\omega )(1-\eta (t,\omega ))}\int_{s=0}^{S}\sigma _{\eta
}(s)W(ds,dt,\omega )  \label{equ_eta_A_bis} \\
Q(p,t,\omega ) &=&\eta (t,\omega )\int_{x=0}^{S}q(x,t,\omega
)dx-\int_{x=0}^{p}q(x,t,\omega )dx\text{ \ \ \ \ \ \ \ \ \ \ \ \ }\forall
p\in \lbrack 0,S]  \label{equ_Q_A}
\end{eqnarray}
...
\textbf{Remarks }Equation (\ref{equ_h_A})\ is a trivial case of the
stochastic evolution equation considered for instance in [CT06] p.130, and,
as such, has a weak solution. To be a fully general stochastic evolution
equation, one could make the coefficients $\mu _{h}(.,p,t)$ and $\sigma
_{h}(.,p,s,t)$ be functionals of $h$ rather than making them functions of $%
h(p,t,\omega )$. We refrain from this generalization in order to get quickly
to the main point.

\bigskip

\textbf{Remark} To make it twice differentiable in price, we build $Q$ from
the ground up, by first defining the log demand delta-density $h$, then
integrating with respect to price to obtain the logarithm of the demand
density $q$. The demand density string $q$ is then positive. Equation (\ref%
{equ_Q_A})\ shows that $Q$ is strictly decreasing in $p$, thus satisfying
assumption (**). We introduced the net demand ratio $\eta (t)$ as an
exogeneous variable in order to avoid having to define the clearing price by
continuation as in (\ref{continuation}). Observe that, by evaluating (\ref%
{equ_Q_A})\ at $p=0$ and $p=S$, we can solve for $\eta $:%
\begin{equation}
\eta (t)=\frac{Q(0,t)}{Q(0,t)-Q(S,t)}  \label{eta}
\end{equation}

In other words it is the ratio of the quantity of all shares available for
puchase over the quantity of shares available for purchase or for sale, and,
as such should lie between zero and one. This is indeed the case for the
double-barrier process in (\ref{equ_eta_A}). With a proper choice of the
parameter $a_{\eta }$ one can even make sure that $\eta (t)$ is strictly
between zero and one almost surely . Examination of (\ref{equ_Q_A}) shows
then that the clearing price $\pi (t)$ satisfying (\ref{pi}) does not reach
...
theorem (theorem 4.1 in \cite{CT06}), there exists $\mathcal{F}_{t}$-adapted
...
d\pi $ is well-defined. We use the Ito-Wentzell formula \cite{Kry09} to calculate
...
satisfied. We then invoke lemma 3.2 in \cite{BB04}, with a slight modification.
Examination of the proof of lemma 3.2 in \cite{BB04} shows that it is not
...
such an arbitrage strategy.\ We examine this question in the context of a
simplified example.
...
motions. It should be clear that the same reasoning applies to a model with
a finite number of Brownian motions. Without loss of generality, we assume
that the drift and volatility coefficients are differentiable functions of
the large trader position, and are not stochastic.%
\begin{eqnarray*}
dP^{(1)}(\theta ^{(1)},t) &=&\mu _{P}^{(1)}(\theta ^{(1)})dt+\sigma
_{P,1}^{(1)}(\theta ^{(1)})dW_{1}(t)+\sigma _{P,2}^{(1)}(\theta
^{(1)})dW_{2}(t) \\
dP^{(2)}(\theta ^{(2)},t) &=&\mu _{P}^{(2)}(\theta ^{(2)})dt+\sigma
_{P,1}^{(2)}(\theta ^{(2)})dW_{1}(t)+\sigma _{P,2}^{(2)}(\theta
^{(2)})dW_{2}(t)
\end{eqnarray*}

For simplicity again, we assume a large trader strategy $\theta \equiv
(\theta ^{(1)},\theta ^{(2)})$ that does not deviate very much from a
straight line. For $0\leq t\leq T$:%
\begin{equation}
\theta ^{(i)}(t)=\theta ^{(i)}(0)(T-t)+\varepsilon \theta _{\Delta }^{(i)}(t)%
\text{ \ \ \ }i=1,2  \label{Small_dev}
\end{equation}

with $\theta _{\Delta }^{(i)}(t)$ bounded continuous predictable processes.
Clearly

\begin{equation*}
V_{t}^{\theta }-V_{0-}^{\theta }=\int_{0}^{t}L(\theta
_{s-},ds)+o_{P}(\varepsilon ^{2})
\end{equation*}%
\bigskip

where $\int_{0}^{t}L(\theta _{s-},ds)$ is defined as in the one stock case.
By Fubini's theorem holds it can be written:

\begin{eqnarray}
\int_{0}^{t}L(\theta ,ds) &=&\int_{0}^{t}\left( \int_{0}^{\theta
^{(1)}(s)}\mu _{P}^{(1)}(x)dx+\int_{0}^{\theta ^{(2)}(s)}\mu
_{P}^{(2)}(x)dx\right) ds+  \label{Lds} \\
&&\int_{0}^{t}S_{1}(s)dW_{1}(s)+\int_{0}^{t}S_{2}(s)dW_{2}(s)  \notag
\end{eqnarray}

where the diffusion coefficient of the proceeds of a fast liquidation
strategy are, for $j=1,2$:

\begin{equation}
S_{j}(s)=\int_{0}^{\theta ^{(1)}(s)}\sigma
_{P,j}^{(1)}(x)dx+\int_{0}^{\theta ^{(2)}(s)}\sigma _{P,j}^{(2)}(x)dx
\end{equation}

We calculate:
\begin{eqnarray*}
S_{j}(s+\varepsilon ) &=&S_{j}(s)+\sigma _{P,j}^{(1)}(\theta
^{(1)})\varepsilon  \\
&&+\sum_{i=1}^{2}\int_{\theta ^{(i)}(s)}^{\theta ^{(i)}(s+\varepsilon
)}\sigma _{P,i}^{(i)}(x)dx \\
&=&\sum_{i=1}^{2}\int_{\theta ^{(i)}(0)(T-s)+\varepsilon \theta _{\Delta
}^{(i)}(s)}^{\theta ^{(i)}(0)(T-s-\varepsilon )+\varepsilon \theta _{\Delta
}^{(i)}(s+\varepsilon )}\sigma _{P,i}^{(j)}(x)dx \\
&=&\varepsilon \frac{d}{dx}\sigma _{P,i}^{(j)}(\theta ^{(i)}(0)(T-s))\theta
_{\Delta }^{(i)}(s)
\end{eqnarray*}

We see that at each time $s$ the diffusion part of (\ref{Lds})\ is zero if:%
\begin{equation*}
\int_{0}^{\theta ^{(1)}(s)}P^{(1)}(x,s)\sigma
_{P,j}^{(1)}(x)dx+\int_{0}^{\theta ^{(2)}(s)}P^{(2)}(x,t)\sigma
_{P,j}^{(2)}(x)dx=0\text{ \ \ \ }j=1,2
\end{equation*}

We assume recursively that:%
\begin{equation*}
b_{1}(s)=-\int_{0}^{\theta ^{(1)}(T-s)}P^{(1)}(x,s)\sigma
_{P,j}^{(1)}(x)dx-\int_{0}^{\theta ^{(2)}(T-s)}P^{(2)}(x,t)\sigma
_{P,j}^{(2)}(x)dx
\end{equation*}

Using our small deviation strategy (\ref{Small_dev}),

The real wealth process satisfies then:
...
\begin{eqnarray*}
V^{\theta }(t)-V^{\theta }(0) &=&\int_{0}^{t}\int_{\theta (0)}^{\theta
(s)}\mu _{P}(x,s)dxds+\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\sigma
_{P}(x,s)dxdW(s)+ \\
&&\int_{0}^{t}N(s)\mu _{C}(s)ds+\int_{0}^{t}N(s)\sigma _{C}(s)dW(s)
\end{eqnarray*}

We choose the number of derivatives so as to cancel the diffusion part:%
\begin{equation*}
N(s)=-\frac{\int_{\theta (0)}^{\theta (s)}\sigma _{P}(x,s)dx}{\sigma _{C}(s)}
\end{equation*}

This results in:%
\begin{equation*}
V^{\theta }(t)-V^{\theta }(0)=\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\mu
_{P}(x,s)-\mu _{C}(s)\frac{\sigma _{P}(x,s)}{\sigma _{C}(s)}dxds
\end{equation*}

Thus, unless for all $x$%
\begin{equation}
\frac{\mu _{P}(x,s)}{\sigma _{P}(x,s)}=\frac{\mu _{C}(s)}{\sigma _{C}(s)}
\label{Noarb}
\end{equation}

We have an arbitrage. Note that condition (\ref{Noarb})\ is the traditional
no-arbitrage constraint. In the liquid case we have neither $\mu _{P}$ nor $%
\sigma _{P}$ depend on the
\begin{equation*}
\lambda (s)=\frac{\mu _{P}(s)}{\sigma _{P}(s)}=\frac{\mu _{C}(s)}{\sigma
_{C}(s)}
\end{equation*}
...
price $\pi $. However, the Bachelier market as considered by \cite{KR09} consists
of only one Brownian motion, and is arbitrage free. This contradiction is
resolved when one realizes that \cite{KR09} do not obtain a clearing price $\pi $
...
In this section, we calibrate the market model and then simulate it in the
risk-neutral measure to price options.. We use empirical high frequency data
of a magnitude of a millisecond. Then the calibrated parameters are utilized
in the discretized simulation model on both physical measure and risk
neutral measure. When simulating the net demand curve $Q$ in risk neutral
measure, the market price of risk equation is solved. We could not find any
case where the market price of risk equation does not have solution.

\bigskip

\textbf{Notation} For any function of time $X(t)$, we define $\Delta
X(t)=X(t+\Delta t)-X(t)$, where the time step $\Delta t$=15 minutes. To
clarify the difference between the calibration phase and the simulation
phase, we will denote observed quantities by $\hat{X}(t)$ and simulated
quantities by $X(t)$.
...
We first discretize the model (\ref{equ_h_A}) to (\ref{equ_Q_A}), in both
time $t$ and price $p$. For simplicity, we set all the drift parameters to
zero, and make the Price takes values in the set $\mathcal{P}=\{0,\Delta
p,2\Delta p,..,S\Delta p\}$ with (HOS) $S=?$.and $\Delta p=\$1.91$. Time
takes values in the set $\mathcal{T}=\{0,\Delta t,2\Delta t,..,T\Delta t\}$
where the time step $\Delta t=?$ (HOS), and the number of observations is
equal to (HOS) $T=?$. We thus use all the dates betwen (HOS) April 1st 9AM??
and April 1st 3PM? The net demand $Q$ at price $p$ is calculated, by
definition as:

\begin{gather*}
\hat{Q}(p,t)=\text{quantity of shares available for purchase at a price }>p
\\
...
and $h(0,t) = h(1,t) = 0$ for computational convenience. The time step $%
\Delta t$ is 15 minutes in this case. The boundary condition of the $q(p,t)$
process, $q(1,t)$, is simulated by

\begin{equation*}
q(1,t) = \sigma_{q_0} \sum_{i=2}^{S} b_{q_{0, i}} z(i,t) \sqrt{\Delta t
\Delta p}
\end{equation*}
where $z(i,t)$ is a random normally distributed number. The rest of the
numerical $q(p,t)$ values are
...
which is consistent with the intuition that $Q(0,t) \geq Q(1,t)$ given $%
q(1,t)\geq 0$. And the rest of the $Q(p,t)$ is

\begin{equation*}
Q(p,t) = [Q(0,t) + \sum_{i=1}^{S}q(i,t)]\eta(t) - [Q(0,t) +
\sum_{i=1}^{p}q(i,t)] \qquad p = 1, 2, \ldots, S
\end{equation*}
where

\begin{eqnarray*}
\Delta \eta(t) &=& \eta(t+\delta t) - \eta(t) \\
&=& a_{\eta}(\bar{\eta} - \eta(t)) \Delta t + \sigma_{\eta} \sqrt{%
\eta(t)(1-\eta(t))} \sum_{i=2}^{S} b_{\eta,h_p} z(i,t) \sqrt{\Delta t } \\
&=& a_{\eta}(\bar{\eta} - \eta(t))\Delta t + \sigma_{\eta} \sqrt{%
\eta(t)(1-\eta(t))} z(S,t) \sqrt{\Delta t }
\end{eqnarray*}
...
\subsection{Simulation Methodology - Real-world Measure ($\mathbb{Q}$
Measure)}
...
\end{equation*}
where $\pi$ is the clearing price, $K$ is the strike level, $r$ is the
interest rate, $T$ is time to maturity and $p$ is the put option price
corresponding to the strike $K$. The implied volatility $\sigma(K)$ is being
solved.

The implied volatility smile generated from the equity options at simulation
period is shown in Figure~\ref{fig::Put_Option_Vols}. The implied
volatilities by simulations under risk neutral measure are plotted as blue
line, which are compared to the real historical implied volatility levels
from Bloomberg. The interest rate level is selected as 0.301\%, and the
dividend yield is set to 0\%. The maturity of the option is in 60 days. The
simulation is evaluated under 500 scenarios.
...
\caption{The implied volatility levels from equity options with maturity of
10 days.}
