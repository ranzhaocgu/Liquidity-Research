...
%TCIDATA{LastRevised=Monday, April 25, 2016 16:35:06}
...
\begin{eqnarray*}
V^{\theta }(t)-V^{\theta }(0) &=&\lim \sum_{i}L(\theta
(s_{i}),s_{i+1})-L(\theta (s_{i}),s_{i})+N(s_{i})\mu _{C}(s_{i})\Delta
s+N(s_{i})\sigma _{C}(s_{i})\Delta W(s_{i}) \\
&=&\lim \sum_{i}\int_{\theta (0)}^{\theta
(s_{i})}P(x,s_{i+1})dx-\int_{\theta (0)}^{\theta
(s_{i})}P(x,s_{i})dx+N(s_{i})\mu _{C}(s_{i})\Delta s+N(s_{i})\sigma
_{C}(s_{i})\Delta W(s_{i}) \\
&=&\lim \sum_{i}\int_{\theta (0)}^{\theta
(s_{i+1})}P(x,s_{i+1})-P(x,s_{i})dx+N(s_{i})\mu _{C}(s_{i})\Delta
s+N(s_{i})\sigma _{C}(s_{i})\Delta W(s_{i}) \\
&=&\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\mu
_{P}(x,s)dxds+\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\sigma
_{P}(x,s)dxdW(s)+\int_{0}^{t}N(s)\mu _{C}(s)ds+\int_{0}^{t}N(s)\sigma
_{C}(s)dW(s)
\end{eqnarray*}

Since $\theta (s)$ is differentiable, we have:%
\begin{eqnarray*}
\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\mu _{P}(x,t)dxds
&=&\int_{0}^{t}\mu _{P}(\theta (s),t)\frac{d\theta (s)}{ds}ds \\
\int_{0}^{t}\int_{\theta (0)}^{\theta (s)}\sigma _{P}(x,s)dxdW(s)
&=&\int_{0}^{t}\sigma _{P}(\theta (s),t)\frac{d\theta (s)}{ds}dW(s)
\end{eqnarray*}

Setting:%
\begin{equation*}
N(s)=-\frac{\sigma _{P}(\theta (s),t)\frac{d\theta (s)}{ds}}{\sigma _{C}(s)}
\end{equation*}

We have:%
\begin{eqnarray*}
V^{\theta }(t)-V^{\theta }(0) &=&\int_{0}^{t}\mu _{P}(\theta (s),t)\frac{%
d\theta (s)}{ds}+N(s)\mu _{C}(s)ds \\
&=&\int_{0}^{t}\mu _{P}(\theta (s),t)\frac{d\theta (s)}{ds}-\frac{\sigma
_{P}(\theta (s),t)}{\sigma _{C}(s)}\frac{d\theta (s)}{ds}\mu _{C}(s)ds \\
&=&\int_{0}^{t}(\mu _{P}(\theta (s),t)-\frac{\sigma _{P}(\theta (s),s)\mu
_{C}(s)}{\sigma _{C}(s)})\frac{d\theta (s)}{ds}ds
\end{eqnarray*}

Thus, unless for all $\theta $:%
\begin{equation}
\mu _{P}(\theta ,s)=\frac{\sigma _{P}(\theta (s),s)\mu _{C}(s)}{\sigma
_{C}(s)}  \label{Noarb}
\end{equation}

We have an arbitrage. Note that condition (\ref{Noarb})\ is the traditional
no-arbitrage constraint. Indeed, the market price of risk is:%
\begin{equation*}
\lambda (s)=\frac{\mu _{P}(s)}{\sigma _{P}(s)}=\frac{\mu _{C}(s)}{\sigma
_{C}(s)}
\end{equation*}

HOMEWORK:\ (for Ran)\ Can you verify why (\ref{Noarb}) works in the
Bachelier market (see Kallsen and Rheinlaender)?
