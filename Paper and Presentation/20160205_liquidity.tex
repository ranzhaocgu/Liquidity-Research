...
And the $h(p,t)$ is the first order difference on $q(p, t)$. Since $q(p,t,\omega ) =\exp{\int_{x=0}^{p}h(x,t,\omega )dx}$, we have
...
Now we have the grids of $\hat{Q}(p,t)$ with $p = 0,1,\ldots, S$ and $\hat{q}(p,t)$ with $p = 1,2,\ldots, S$ and $\hat{h}(p,t)$ with $p = 2,3,\ldots, S$. Let $HE$ matrix as

$$
\left(
  \begin{array}{cccc}
    dh_{2,0} & \cdots & dh_{S,0} & \frac{d\eta_{0}}{\sqrt{\eta_{0}(1-\eta_{0})}} \\
    \vdots & \ddots & \vdots & \vdots \\
    dh_{2,T} & \cdots & dh_{S,T} & \frac{d\eta_{T}}{\sqrt{\eta_{T}(1-\eta_{T})}}  \\
  \end{array}
\right)
$$

Calculate the instantaneous volatilities $\sigma_h$ and the correlation matrix $\hat{R}dt = Corr(HE)$. And take Cholesky decomposition with respect to $\hat{R}$. That is,
...
\begin{eqnarray*}
dh(p,t) &=& h_p(t+\delta t) - h_p(t) = [...]dt + \sigma_{h_p} \Sigma_{i=2}^{S} b_{h_{p, i}} z(i,t) \sqrt{\delta t \delta p} \\
        &=& \sigma_{h_p} \Sigma_{i=2}^{S} b_{h_{p, i}} z(i,t) \sqrt{\delta t \delta p} \qquad p = 2, 3, \ldots, S
\end{eqnarray*}

and $h(0,t) = h(1,t) = 0$ for computational convenience. The boundary condition of the $q(p,t)$ process can be computed from $q(1,t)$ and $q(2,t)$. To be specific,

$$
q(1,t) = \max(0, q(2,t) - q(2,t)h(2,t))
$$

And the rest of the numerical $q(p,t)$ values are

$$
q(p,t) = q(1,t) + \exp{\Sigma_{i=2}^{S}h_x(t)} \qquad p = 2, 3, \ldots, S
$$

Then the boundary condition of the net demand $Q(0,t)$ is calculated as

$$
Q(0,t) = \max(Q(1,t), Q(1,t) - Q(1,t)q(1,t))
$$

and the rest of the $Q(p,t)$ is

$$
Q(p,t) = [Q(0,t) + \Sigma_{i=1}^{S}q(i,t)\eta(t)] - [Q(0,t) + \Sigma_{i=1}^{p}q(i,t)\eta(t)]  \qquad p = 1, 2, \ldots, S
$$
where

\begin{eqnarray*}
d\eta(t) &=& \eta(t+\delta t) - \eta(t) = a_{\eta}(\bar{\eta} - \eta(t)) dt + \sigma_{\eta} \sqrt{\eta(t)(1-\eta(t))}  \Sigma_{i=2}^{S} b_{\eta,h_p} z(i,t) \sqrt{\delta t } \\
...
$$
dh(p,t) = \sigma_{h_p} \Sigma_{i=2}^{S} b_{h_{p, i}} [z(i,t) \sqrt{\delta t \delta p} - \lambda(i)\delta t \delta p] \qquad p = 2, 3, \ldots, S
$$
...
\begin{eqnarray*}
&&\left( Q(0,t)\sigma _{Q_{0}}b_{Q_{0},s}+q(1,t)\sigma _{q_{1}}b_{q_{1},s}+\sum_{y=3}^{S}\exp
(\sum_{i=2}^{p-1}h_{i}(t))\sum_{x=2}^{p-1}h_{x}(t)\sigma_{h_{x}}b_{h_{x,y}}\right) \eta (t)+ \\
&&\left( [Q(0,t)+\sum_{x=1}^{S}q(x,t)]\sigma _{\eta }\sqrt{\eta (t)(1-\eta
(t))}\right) b_{\eta ,s}+ \\
&&\left( Q(0,t)\sigma_{Q_{0}}b_{Q_{0},s}+q(1,t)\sigma_{q_{1}}b_{q_{1},s}+\sum_{y=3}^{p}\exp
(\sum_{i=2}^{p-1}h_{i}(t))\sum_{x=2}^{p-1}h_{x}(t)\sigma_{h_{x}}b_{h_{x,y}}\right)
\end{eqnarray*}
