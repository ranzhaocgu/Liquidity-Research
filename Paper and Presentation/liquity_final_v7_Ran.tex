...
(\cite{BB04,BK15a, BK15b,Fre98, Jar92, Jar94, PS98a, PS98b, RS10, SW00}) 
considers the action of a large trader who can
...
modification of the net demand curve to $Q(p_{1}-dp,t_{1}+dt)=Q(p_{1},t_{1})-%
...
{ass::decrease_net_demand} is similar to Assumption A3 in \cite{Jar92} or
...
_{P}(x,t)\int_{s=0}^{S}b_{P}(x,s,t)W(ds,dt)\right) +\frac{1}{2}\frac{\partial
...
The differential of the right side of (\ref{eqn::demand_and_price})\ is clearly
...
\begin{equation}
\mathbb{E}\left[\exp (\int_{t=0}^{T}\int_{s=0}^{S}\lambda (s,t)W(ds,dt)-\frac{1}{2}%
\int_{t=0}^{T}\int_{s=0}^{S}\lambda ^{2}(s,t)dsdt)\right]=1  \label{Novikov}
\end{equation}
\end{assumption}
...
measure $\mathbb{Q}$ for $P(x,t)$. By assumption \ref{ass::theta_semimartingale} 
we can invoke (\ref%
...
\begin{eqnarray}
\bar{\theta}^{(1)} &=&-2\frac{\sigma_{P,2}^{(1)}\left(\sigma_{P,1}^{(2)} \right)^2}
{\sigma_{P,2}^{(1)}\left(\sigma_{P,1}^{(1)}\right)^2}  \label{theta_bar_1} \\
...
\begin{eqnarray*}
V^{\bar{\theta}}(t)-V^{\bar{\theta}}(0_{-}) &=&\int_{u=0}^{t}L(\bar{\theta},du)
\\
&=&\int_{u=0}^{t}\mu _{P}^{(1)}\bar{\theta}^{(1)}(u)+\mu _{P}^{(2)}\bar{\theta}%
^{(2)}(u)du \\
...
\begin{eqnarray}
\int_{s = \delta _{1}}^{S}b_{h}^{2}(p,s)ds &=&1\text{\ \ \ \ \ \ \ }\forall p\in
\lbrack 0,S]  \label{bh} \\
\int_{s = 0}^{\delta _{2}}b_{\eta }^{2}(s)ds &=&1  \label{b_eta}
\end{eqnarray}

and that the bounded linear operator $B_{h}:E\rightarrow E$ given by%
\begin{equation*}
(B_{h}f)(p)=\int_{u=0}^{S}b_{h}(p,u)f(u)du
\end{equation*}%
\bigskip
...
{equ_Q_A}) shows then that, if $P(x,t)$ satisfies (\ref{eqn::demand_and_priceItoWE}), then it
...
\begin{proposition}
Our model satisfies Assumptions~\ref%
...
\begin{equation} %\label{eqn::d_sigma_b_d_p}
...
bounded. By Theorem 7.19 in \cite{Kun97}, Novikov's criterion holds.
