...
%TCIDATA{LastRevised=Friday, August 28, 2015 15:47:39}
...
\begin{enumerate}
  \item Calculate the estimated excess demand $\hat{Q}(p,t)$ for $p$ and $t$, from the market data;
  \item Calculate $\hat{\eta}(t)=\frac{\hat{Q}(0,t)}{\hat{Q}(0,t)-\hat{Q}(S,t)}$
  \item Estimate the mean and variance of $\hat{\eta}$;
  \item Using \cite{equ_Q_A}, calculate the series of $\hat{q}(p,t)$ by taking first order difference on $Q(p,t)$. \\
            $\hat{q}(p,t) = \frac{\partial}{\partial p}\hat{Q}(s,t)\cdot \eta(t) =\- \hat{Q}(p,t)$
  \item Choose $f(p)$ (here as what I chose for simulation);
  \item Calculate series $\hat{h}(p,t)$ from $\hat{q}(p,t)$;
  \item Estimate the mean and variance of $h$;
  \item Simulate $Q(p,t)$ by $n$ paths, and estimate the variance-covariance matrix of $Q$;
  \item Compare the variance-covariance matrix of simulated $Q(p,t)$ with out-of-sample $Q$'s variance and covariance matrix.
\end{enumerate}
